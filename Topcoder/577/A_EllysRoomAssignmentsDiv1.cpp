...
double EllysRoomAssignmentsDiv1::getAverage(vector <string> a)
{
	int m = a.size (), n;
	string s;
	...
	for (i = 0; i < n; i += R) {
		if (n - i < R) {
			if (f) {
				//printf ("here %d %d %d %d %.3lf %.3lf\n", n, R, RR, -b[n-1],ans/np, (ans -b[n-1])/(np+1));
				//printf ("%.3lf\n", 2.0/3*ans/np + 1.0/3*(ans-b[n-1])/(np+1));
				double sum = ans;
				ans = ((double)R - RR) / R * sum / np;
				for (j = i; j < n; ++j) {
					ans += 1.0 / R * (sum - b[j]) / (np + 1);

				}
				return ans;
			}
			else return (ans - ra) / (np + 1);
		}
		else {
			double s = .0;
			for (j = i; j < i + R; ++j)
